...
// v1 taxonomy of BLE Device Types. DeviceTypeUnknown is what
// ClassifyDeviceType returns when none of the three signals resolve a more
// specific type — same naming convention as core/engine.DeviceTypeUnknown,
// and the same "no accuracy bar for v1" stance: classification is
// best-effort, and an incorrect guess is an acceptable outcome.
const (
...
// 16-bit Appearance value; category = value >> 6) to the v1 taxonomy above.
// Deliberately not exhaustive — since classification is best-effort, only
// categories with an unambiguous mapping to one of the eight v1 types are
// included; every other category falls through to the next signal
// (ServiceUUIDs, then Name keywords).
...
// Appearance category field. The encoding is a deliberate convention:
// discovery/blescan.toAdvertisement, the only write site, formats a raw
// 16-bit Appearance value as a 4-digit lowercase hex string with no "0x"
// prefix (e.g. "03c0") — documented identically there.
...
// Service). Deliberately kept small and defensible rather than exhaustive.
// Checked in this fixed order — first match wins — so priority doesn't
// depend on slice/map iteration order.
...
// last-resort signal, matched fuzzily. Checked in this fixed order for the
// same reason as serviceUUIDRules.
...
// ClassifyDeviceType assigns a best-effort DeviceType to a fully
...
// Name/Vendor/DistanceEstimate are already set — never inside
